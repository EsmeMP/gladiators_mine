...
def ventas():
    if 'username' in session and 'rol' in session:
        username = session['username']
        rol = session['rol']
        if rol or request.endpoint == 'ventas':  # Administradores y cajeros
            return render_template('Regventa.html', username=username, rol=rol)
        else:
            flash("No tienes permisos para acceder a esta sección")
            return redirect(url_for('index'))
    else:
...
#         )
#         cursor = connection.cursor()
#         query = "SELECT username, rol FROM usuario WHERE username = %s AND password = %s"
#         cursor.execute(query, (username, password))
#         resultado = cursor.fetchone()

#         if resultado:
#             return resultado   # Devuelve (username, rol) donde rol es booleano
#         else:
#             return None
...
#                 username = serializer.loads(remember_token, salt='remember-me', max_age=60*60*24*30)
#                 session['username'] = username
#                 # Aquí podrías cargar más datos de usuario si fuera necesario
#             except:
#                 pass
...
#             return render_template('secciones.html', username=username, rol=rol)
#         else:
#             # Mostrar contenido para cajeros o redirigir
#             flash("No tienes permisos para acceder a esta sección")
#             return redirect(url_for('index'))
#     else:
#         return redirect(url_for('secciones'))
...
#         usuario = {}
#         if usuario_general:
#             # Supongamos que el resultado es una tupla con nombres de columna
